...
export class UserController {
  ...
  @Get('list')
  async list(): Promise<{ code: number; msg: string; data: FrontendUserDto[] }> {
    ...
    return { code: 200, msg: '用户列表获取成功', data: toFrontendListDtos(users) };
  }
  ...
  async get(@Param('publicId') publicId: string) {
    ...
    if (user) {
      ...
      this.loggingService.log('GET /system/user/get failed', { responseDescriptor: { data: null } });
